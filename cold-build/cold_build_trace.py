...
// === TRAZA cold-build (iceberg nivel 1): exports nativos de creacion de caso (por RVA v19) ===
var CASE_RVA = {'Go_JobCaseInit':0x2203BE0, 'Go_JobCaseLoadSet':0x21FC9C0,
                'Go_JobCaseStaticLin':0x21FCBA0, 'Go_JobCaseSetAllToRun':0x2202EE0};
if (m) Object.keys(CASE_RVA).forEach(function(nm){
  var ep = m.base.add(CASE_RVA[nm]);
  Interceptor.attach(ep, {
    onEnter:function(a){
      this.nm=nm;
      if(nm==='Go_JobCaseStaticLin'){
        this.nl=a[3]; this.jc=a[4]; this.ab=a[5]; this.num=a[6];
        var s={type:'cjob',nm:nm,ph:'in'};
        try{s.tag=a[0].readS32();}catch(e){}
        try{s.nLoadDim=this.nl.readS32();}catch(e){}
        try{s.jcase=this.jc.readS32();}catch(e){}
        try{s.acase=a[1].readAnsiString();}catch(e){ try{s.acase=a[1].readUtf16String();}catch(e2){} }
        send(s);
      } else if(nm==='Go_JobCaseLoadSet'){
        this.nl=a[1]; this.num=a[2];
        var s={type:'cjob',nm:nm,ph:'in'}; try{s.nLoad=this.nl.readS32();}catch(e){} send(s);
      } else {
        this.num=a[1]; var s={type:'cjob',nm:nm,ph:'in'}; try{s.tag=a[0].readS32();}catch(e){} send(s);
      }
    },
    onLeave:function(r){
      var s={type:'cjob',nm:this.nm,ph:'out'};
      if(this.nm==='Go_JobCaseStaticLin'){
        try{s.nLoadDim=this.nl.readS32();}catch(e){}
        try{s.jcase=this.jc.readS32();}catch(e){}
        try{s.ab=this.ab.readS32();}catch(e){}
        try{s.num=this.num.readS32();}catch(e){}
      } else if(this.nm==='Go_JobCaseLoadSet'){ try{s.nLoad=this.nl.readS32();}catch(e){} }
      else { try{ if(this.num) s.num=this.num.readS32(); }catch(e){} }
      send(s);
    }
  });
});
...
class Probe:
    ...
    def _m(self, msg, data):
        if msg.get("type") == "send":
            ...
            elif p.get("type") == "cjob":
                extra = "  ".join(f"{k}={v}" for k,v in p.items() if k not in ("type","nm","ph"))
                print(f"   [CJOB {p['nm']}.{p.get('ph')}] {extra}")
            elif p.get("type") == "log":
                print(f"   [JS] {p.get('m')}")
